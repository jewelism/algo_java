 ...
 * 상담원으로 일하고 있는 백준이는 퇴사를 하려고 한다.
 ...
 N = 7인 경우에 다음과 같은 상담 일정표를 보자(1일~N일)
...
public class p14501_retire {
    public static void main(String[] args) {
        ...
        int N = scan.nextInt();
        int[] T = new int[N+1]; //상담을하는데 걸리는 기간
        int[] P = new int[N+1]; //상담했을때 받을수있는 금액
        int[] result = new int[N+1]; //각 일수로 부터 상담을 시작했을때, 총금액을 저장할 배열
        for(int i=1; i<=N; i++){
            T[i] = scan.nextInt();
            P[i] = scan.nextInt();
        }
        for(int i=1; i<=T.length; i++){
            if(N+T[i]>=N+1){ //이조건은 회사에없기때문에 상담을 할수가없다
